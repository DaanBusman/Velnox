...
export default async function Page() {
  ...
  const workloads = await listWorkloads({ kind: 'QEMU' });

  if (!workloads.ok) {
    return (
      <>
        <PageHeader title={t('nav.virtualMachines')} />
  ...
  return (
    <>
      <PageHeader title={t('nav.virtualMachines')} description={t('inventory.vmPageDescription')} />
      <WorkloadTable
        workloads={visible}
